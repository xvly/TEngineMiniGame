...
namespace TEngine.Editor.Inspector
{
    ...
    internal sealed class ProcedureSettingInspector : GameFrameworkInspector
    {
        ...
        // 并在类中添加以下辅助方法：
        private static string[] GetProcedureTypeNamesFromAsmdef(string asmdefName)
        {
            ...
            foreach (var asm in assemblies)
            {
                try
                {
                    if (!string.Equals(asm.GetName().Name, asmdefName, System.StringComparison.OrdinalIgnoreCase))
                    {
                        continue;
                    }
                    ...
                    foreach (var t in types)
                    {
                        ...
                        if (typeof(ProcedureBase).IsAssignableFrom(t))
                        {
                            list.Add(t.FullName);
                        }
                    }
        ...
        {
            _procedureTypeNames = GetProcedureTypeNamesFromAsmdef("Core");
